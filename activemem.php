  ...
  <div class="rounded">
    <main class="container clear"> 
      ...
      <div id="content" class="three_quarter"> 
        <!-- ################################################################################################ -->
        <h1 style = "color: black;">Number of Active Members</h1>
        <p>- Statistics are based on user IP address and whether its last visit date is during current month</p>
        
        <h1 style = "color: black;">This Month</h1>
          <table>
            <thead>
        ...
        echo "<p>- Current Month is <span style = 'color: blue;'>$t</span></p>";

        $today = date('m');
        $current_year = date('Y');
				$sql = "SELECT count(users_id) as total_visitor, month(last_login) as month FROM `users`where year(last_login) = '$current_year' and month(last_login) = '$today'";
        ...
        if($result->num_rows > 0){
          ...
          <h1 style = "color: black;">Monthly</h1>
          <table>
            <thead>
              <tr style = 'text-align: center;'>
                <th>Month</th>
        ...
        $today = date('m');
        $current_year = date('Y');
        echo "<p>- Monthly Active Members in current year, <span style = 'color: blue;'>$current_year</span></p>"; 
        $sql = "SELECT count(users_id) as total_visitor, month(last_login) as month FROM users where year(last_login) = '$current_year' group by month(last_login)";
        $result = mysqli_query($con, $sql);
        if($result->num_rows > 0){
          while($row = $result->fetch_assoc()){
            echo "<tr style = 'text-align: center;'>";
      ...
      ?>
        ...
        $today = date('m');
        $sql = "SELECT count(users_id) as total_visitor, year(last_login) as year FROM `users` WHERE year(last_login) <> '0000' group by year(last_login)";
        $result = mysqli_query($con, $sql);
        if($result->num_rows > 0){
          while($row = $result->fetch_assoc()){
            echo "<tr style = 'text-align: center;'>";
