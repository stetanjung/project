...
<div class="wrapper row3">
  <div class="rounded">
    <main class="container clear"> 
      ...
      ?>
        ...
        ?>
        <form action="member-page-return.php" method="POST">
        <table>
          <tr>
            <td>Title</td>
            <td>Year</td>
        ...
        <?php
        $users_id=$_SESSION['users_id'];
        $query=mysqli_query($con,"SELECT b.*,r.borrows_id,r.borrow_date,r.return_date FROM books b, borrows r WHERE b.books_id = r.books_id AND r.users_id='$users_id'");

        while($row=mysqli_fetch_array($query)){
          //ALL THE SQL STUFFS
          if($row['return_date']!=NULL)continue;
  ...
  $books_id=$row['books_id'];
          $query_book=mysqli_query($con,"SELECT title FROM books WHERE books_id='$row[books_id]'");
          $fetch_book=mysqli_fetch_array($query_book);
          $expected=date('Y/m/d',strtotime($row['borrow_date'].'+ 7 days'));
          // BORDER -------------------------------
          //ALL THE CONTENT IN TABLE
          echo "
          <tr>
