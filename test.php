<?php
	...
	$user = "create table users(users_id char(6) not null,	username varchar(30) not null,	password varchar(30) not null,	first_name varchar(30) not null,	last_name varchar(30) not null, date_of_birth date not null,	parents_id char(5),	primary key (users_id),	foreign key (parents_id) references parents(parents_id))";
	...
	$borrow = "create table borrows(	borrows_id char(6) not null,	users_id char(6),	books_id char(6),	borrow_date date,	return_date date, borrow_number_days int,	overdue char(5),	primary key(borrows_id),foreign key (users_id) references users(users_id), foreign key (books_id) references books(books_id))";
